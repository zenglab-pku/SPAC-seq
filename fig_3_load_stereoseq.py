...
# Change in 2023-12-10: image_file is now an optional parameters
...
def load_cell(
    ...
    # exchange gem xy!
    dat = dat.rename(columns={'x': 'temp', 'y': 'x'})
    ...
    tissue_hires_scalef = max((mask['x'].max()+1)/image.shape[1],(mask['y'].max()+1)/image.shape[0])
